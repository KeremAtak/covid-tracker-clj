...
The `home-routes` handler in the `covid-tracker-clj.routes.home` namespace
...
The `home-page` function will in turn call the `covid-tracker-clj.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `covid-tracker-clj.handler` namespace:
...
Request middleware functions are located under the `covid-tracker-clj.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `covid-tracker-clj.dev-middleware` namespace found in
